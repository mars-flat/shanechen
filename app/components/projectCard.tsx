...
export default function ProjectCard({
  ...
  return (
    <div className={`shrink-0 w-[50vh] h-full relative flex justify-center items-center group snap-center snap-always ${roboto.className}`}>
      ...
      <div className="absolute top-0 left-0 bg-lime-50 w-full h-full opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none drop-shadow-xl">
      </div>

      <div className="bg-purple-100 w-[95%] h-[97%] drop-shadow-sm flex flex-col items-center justify-between text-gray-800">
        <div className="w-[50%] overflow-hidden flex items-start py-[8%]">
          <Image src={imageUrl} alt={title} height={imageHeight} width={imageWidth} className="rounded-lg"/>
        </div>
        <Link href={link} target="_blank" 
        className={`w-[90%] text-3xl font-bold break-words mb-[2%] flex justify-start items-center hover:text-gray-600 transition-color duration-300 ${roboto.className}`}>
          {`${title}`}&nbsp;<FaLink className="text-xs text-gray-400 xl:opacity-0 xl:group-hover:opacity-100 transition-opacity duration-500" />
        </Link>
        <p className={`w-[90%] text-lg italic text-gray-500 ${roboto.className}`}>{subtitle}</p>
        <div className="w-[70%] h-[0.2%] bg-gray-400 group-hover:bg-black transition-color duration-500 my-[3%]"></div>
        <p className={`w-[90%] text-md ${roboto.className}`}>{desc}</p>
        <div className="w-[70%] h-[0.2%] bg-gray-400 group-hover:bg-black transition-color duration-500 my-[3%]"></div>
